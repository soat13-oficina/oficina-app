...
import br.com.oficina.pecainsumo.application.usecase.AdicionarEstoquePecaUseCase;
...
@Service
public class AdicionarEstoquePecaService implements AdicionarEstoquePecaUseCase {
    private final PecaInsumoRepository pecaInsumoRepository;

    public AdicionarEstoquePecaService(PecaInsumoRepository pecaInsumoRepository) {
        this.pecaInsumoRepository = pecaInsumoRepository;
    }

    @Override
    public void adicionarEstoque(AdicionarEstoquePecaCommand command) {
        if (command.quantidade() <= 0) {
            throw new RegraDeNegocioException("A quantidade a ser adicionada deve ser maior que zero");
        }
